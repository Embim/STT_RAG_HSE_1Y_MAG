...
> - **Реализация поэтапная, с ревью между фазами.** Фаза 1: контейнеризация API (+ lazy-init фикс) + nginx + TLS + Angular-поиск. Фаза 2: ingest-UI + **async-jobs**. Фаза 3: WireGuard + security-hardening + сетевой рунбук.
> - **Дашборды (Grafana/MLflow/Langfuse/Airflow/Weaviate) — только через VPN.** Публично — лишь сайт (поиск + ингест). Снимаем публикацию портов админ-сервисов (§11, риск #2).
> - **Ингест — асинхронный** (`202 {job_id}` + `GET /ingest-status/{id}` + поллинг Angular, §10). Решает 504 и даёт прогресс-UX.
> - **Cloudflare Turnstile на `/ingest*`** (§11, риск #5/#6) — включаем. Учесть CSP-origin Turnstile в nginx и зависимость во фронте.
>
